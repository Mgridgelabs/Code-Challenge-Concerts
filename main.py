...
def add_sample_data(session):
    # Create band and venue first
    band = Band(name='The Beatles', hometown='westlands')
    venue = Venue(title='The mall', city='Nairobi')
    
    # Add band and venue to session first
    session.add(band)
    session.add(venue)
    session.commit()  # Commit to generate IDs
    
    # Now create a concert with existing band and venue
    concert = Concert(date='2024-09-15', band_id=band.id, venue_id=venue.id)
